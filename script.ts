// Initializing the progress bar so it does not reset to zero every time the function is run
...
function raindrops(): void {
  ...
  for (let i = 0; i < randomNumber(6, 8); i++) {
    ...
    newDrop.onclick = function () {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          ...
          // After all items are faded, return visibility
          setTimeout(function () {
    ...
    let container: HTMLElement | null = document.getElementById('drop');
    ...
    function abGeneration(target: number): number[] {
    ...
    let sumArray = abGeneration(parseInt(firstNumber, 10));
...
function checkOverlap(left: number, top: number, imgLocations: { left: number; top: number }[]): boolean {
  // loop through location array
  for (var i = 0; i < imgLocations.length; i++) {
    // set to current i
    var imgLocation = imgLocations[i];
    // check delta of x and y locations
    var xOverlap = left - imgLocation.left;
    var yOverlap = top - imgLocation.top;
